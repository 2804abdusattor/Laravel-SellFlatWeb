<footer class="py-3 my-4">
    <ul class="nav justify-content-center border-bottom pb-3 mb-3">
      <li class="nav-item"><a href="/" class="nav-link px-2 text-muted">Главная страница</a></li>
      <li class="nav-item"><a href="/about" class="nav-link px-2 text-muted">Про нас</a></li>
      <li class="nav-item"><a href="/contact" class="nav-link px-2 text-muted">Контакты</a></li>
    </ul>
    <p class="text-center text-muted"><a href="#">Вк</a></p>
    <p class="text-center text-muted"><a href="#">Facebook</a></p>
    <p class="text-center text-muted"><a href="#">Twitter</a></p>
    <p class="text-center text-muted">© 2022 Company, Sale-Appartment</p>
</footer>